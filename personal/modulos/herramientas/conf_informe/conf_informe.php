...
$consulta = "SELECT `id_informe`, `tipo`, `titulo`, `pregunta`, `aclaracion`, `respuestas_posible`, `modalidad`, `id_titologeneral` FROM `confi_informe_$cicloLectivo` WHERE `id_titologeneral`='$id_titulo'";
$resultado = $conexion->prepare($consulta);
$resultado->execute();
$data=$resultado->fetchAll(PDO::FETCH_ASSOC);
...
function eliminar_FINAL(){
     ...
     dataFila.push(3);
        ...
        $.ajax({
            url: "modulos/herramientas/conf_informe/elementos/crud_pregunta.php",
            type: "POST",
